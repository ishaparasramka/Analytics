...
fit2 = lm(sales ~ price + promotion, data=omni) #sales dependent variable aand price and promo is independent

# summary statistics of model IMP STEP
summary(fit2)

fitted(fit2) #these steps help us understand the diffs between the variables given and predicted
omni$sales
residuals(fit2)
summary(residuals(fit2)) #diff should not be high
...
par(mfrow=c(2,2)) #dividing the region into 2x2 matrix to see the plots together
plot(fit2) #4 plots are drawn 
par(mfrow=c(1,1)) #back again to see only one graph
plot(fit2)
...
plot(fit2,4)  # tells outliers which can affect model #row numbers that have extreme values
omni[-c(11,14,15),] #remove the values
...
fit2
summary(fit2)
head(omni) #we want to find the predicted values
cbind(omni,predict(fit2, newdata = data.frame(omni$price, omni$promotion))) #instead of typing this whole we can use the following formula
