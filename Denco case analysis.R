...
#read file : Method1
sales1 = read.csv("./Data/denco.csv") # subfolder in the repository where the file exists #. means the current folder
str(sales1) #see if data is loaded and check its structure
head(sales1)

#read file : Method2 : when location is not in project folder
sales2 = read.csv(file.choose()) # now you can go to the path where the file is stored
str(sales2)
head(sales2)
...
# whichever you read the data, store it in a new variable-- sales object
...
#dplyr
names(sales) #names of the columns
...
class(t1); length(t1)
head(t1) # top 6 but not sorted thus not showing the top 6 
...
#Case-2b : Customer- least Revenue----
...
sales %>% filter(revenue >= 700000 ) %>% select(custname, revenue,region) %>% 
